...
%Menghitung error
Error=[];
disp('Menghitung Error')
for i=2:length(data)
    Error(i) = (abs((data(i) - (forecasting_akhir(i))))/(data(i)))*100;
    disp([num2str(i) '.' ' Nilai error : ' num2str(Error(i))])
end
total = 0;
for i=1:size(Error,1)
  for j=1:size(Error,2)
    total = total + Error(i,j);
  end
end
total = sum(Error(:))
rata_rata_error= total/length(data)
sum(Error);
% figure()
% plot(data(2:end))
% hold on
